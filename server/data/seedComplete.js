/**
 * TripWise - Complete Database Seed Script
 * 
 * Seeds ALL collections with demo data for project review/presentation.
 * Run: node server/data/seedComplete.js
 * 
 * Creates:
 *   - 2 Users (1 admin, 1 traveler)
 *   - 6 Destinations (world locations)
 *   - 6 Hotels (one per destination)
 *   - 10 Attractions
 *   - 6 Packages
 *   - 4 Bookings (for demo traveler)
 *   - 2 Trips (AI-planned itineraries)
 *   - 3 Notifications
 */
...
// ─── IMAGE URLS (verified working Unsplash URLs) ────────────────────────────
...
const destinations = [
  {
    ...
    travelStyles: ['Luxury', 'Romantic', 'Relaxation'], travelerTypes: ['Couple', 'Solo'],
  ...
  },
  {
    ...
    travelStyles: ['Cultural', 'Historical'], travelerTypes: ['Family', 'Couple', 'Solo'],
  ...
  },
  {
    ...
    travelStyles: ['Adventure', 'Relaxation', 'Nature'], travelerTypes: ['Couple', 'Friends', 'Solo'],
    popularFor: ['Beaches', 'Surfing', 'Yoga'], rating: 4.7
  },
  {
    ...
    travelStyles: ['Luxury', 'Romantic', 'Culinary'], travelerTypes: ['Couple', 'Family'],
    popularFor: ['Coastal Views', 'Food', 'Boating'], rating: 4.9
  },
  {
    ...
    travelStyles: ['Luxury', 'Relaxation'], travelerTypes: ['Couple', 'Family'],
    popularFor: ['Scuba Diving', 'Resorts', 'Beaches'], rating: 5.0
  },
  {
    ...
    travelStyles: ['Luxury', 'Adventure', 'Shopping'], travelerTypes: ['Family', 'Couple', 'Friends'],
    popularFor: ['Skyscrapers', 'Shopping', 'Desert Safari'], rating: 4.7
  }
];
...
const attractions = [
  { name: 'Oia Sunset Point', destination: 'Santorini', description: 'Famous viewpoint to watch the sunset over the caldera.', entryFee: 0, openingTime: 'Open 24/7', closingTime: 'Open 24/7', category: 'Nature', rating: 4.9, priority: 1 },
  ...
  { name: 'Fushimi Inari Shrine', destination: 'Kyoto', description: 'Iconic shrine famous for thousands of vermillion torii gates.', entryFee: 0, openingTime: 'Open 24/7', closingTime: 'Open 24/7', category: 'Temple', rating: 4.9, priority: 1 },
  ...
  { name: 'Burj Khalifa', destination: 'Dubai', description: 'The tallest building in the world at 828 meters.', entryFee: 3500, openingTime: '8:30 AM', closingTime: '11:00 PM', category: 'Architecture', rating: 4.9, priority: 1 },
  { name: 'Dubai Mall', destination: 'Dubai', description: 'One of the largest shopping malls in the world.', entryFee: 0, openingTime: '10:00 AM', closingTime: '12:00 AM', category: 'Shopping', rating: 4.7, priority: 2 },
  { name: 'Banana Reef', destination: 'Male', description: 'One of the most famous dive sites in the Maldives.', entryFee: 5000, openingTime: '6:00 AM', closingTime: '6:00 PM', category: 'Nature', rating: 4.8, priority: 1 },
  { name: 'Path of the Gods', destination: 'Amalfi Coast', description: 'A breathtaking coastal hiking trail with stunning Mediterranean views.', entryFee: 0, openingTime: 'Open 24/7', closingTime: 'Open 24/7', category: 'Nature', rating: 4.9, priority: 1 }
];
...
async function seedComplete() {
  try {
    ...
    console.log('[1/8] MongoDB connected for seeding');
    ...
    console.log('[2/8] 2 users seeded (admin + traveler)');

    // ── 2. Destinations ───────────────────────────────────
    await Destination.deleteMany();
    const insertedDestinations = await Destination.insertMany(destinations);
    console.log(`[3/8] ${insertedDestinations.length} destinations seeded`);

    // ── 3. Hotels ─────────────────────────────────────────
    await Hotel.deleteMany();
    await Hotel.insertMany(hotels);
    console.log(`[4/8] ${hotels.length} hotels seeded`);

    // ── 4. Attractions ────────────────────────────────────
    await Attraction.deleteMany();
    await Attraction.insertMany(attractions);
    console.log(`[5/8] ${attractions.length} attractions seeded`);

    // ── 5. Packages ───────────────────────────────────────
    await Package.deleteMany();
    const findDest = (name) => insertedDestinations.find(d => d.name === name);

    const packages = [
      {
        title: 'Romantic Santorini Getaway',
        destination: findDest('Santorini')._id,
      ...
      },
      {
        ...
        destination: findDest('Bali')._id,
        durationDays: 7, durationNights: 6,
        price: 45000, discountPrice: 42000,
        description: 'Explore the lush rice terraces, ancient temples, and beautiful beaches of Bali.',
        image: 'https://images.unsplash.com/photo-1537996194471-e657df975ab4?w=800&fit=crop&q=80',
      ...
      },
      {
        ...
        destination: findDest('Kyoto')._id,
      ...
      },
      {
        ...
        destination: findDest('Dubai')._id,
        durationDays: 4, durationNights: 3,
        price: 55000, discountPrice: 49000,
        description: 'Experience the glitz and glamour of Dubai with desert safari and luxury accommodation.',
        image: 'https://images.unsplash.com/photo-1512453979798-5ea266f8880c?w=800&fit=crop&q=80',
      ...
      },
      {
        ...
        destination: findDest('Male')._id,
        durationDays: 5, durationNights: 4,
        price: 120000, discountPrice: 110000,
        description: 'Escape to crystal-clear waters and pristine white sand beaches with overwater villa.',
        image: 'https://images.unsplash.com/photo-1514282401047-d79a71a590e8?w=800&fit=crop&q=80',
      ...
      },
      {
        ...
        destination: findDest('Amalfi Coast')._id,
        durationDays: 6, durationNights: 5,
        price: 105000, discountPrice: 95000,
        description: 'Drive along the stunning Amalfi coastline, explore charming villages, and indulge in Italian cuisine.',
        image: 'https://images.unsplash.com/photo-1533587851505-d119e13fa0d7?w=800&fit=crop&q=80',
    ...
    console.log(`[6/8] ${insertedPackages.length} packages seeded`);

    // ── 6. Bookings (for demo traveler) ───────────────────
    ...
    const bookingsData = [
      {
        ...
        itemType: 'Package',
        itemId: insertedPackages[0]._id,
        itemName: 'Romantic Santorini Getaway',
        itemImage: 'https://images.unsplash.com/photo-1613395877344-13d4a8e0d49e?w=800&fit=crop&q=80',
        destinationName: 'Santorini, Greece',
        checkIn: new Date(now.getTime() + 15 * 24 * 60 * 60 * 1000),
        checkOut: new Date(now.getTime() + 20 * 24 * 60 * 60 * 1000),
        travelers: 2,
        travelerDetails: { firstName: 'Rahul', lastName: 'Sharma', email: 'traveler@tripwise.com', phone: '+91 9123456789' },
        baseFare: 79000,
        taxesAndFees: 9480,
        discount: 5000,
        totalAmount: 83480,
        bookingRef: 'TWSNT2025A',
        status: 'Confirmed',
        paymentStatus: 'Paid'
      },
      {
        ...
        itemName: 'Ubud Hanging Gardens',
        itemImage: 'https://images.unsplash.com/photo-1571896349842-33c89424de2d?w=600&fit=crop&q=80',
        destinationName: 'Bali, Indonesia',
        checkIn: new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000),
        checkOut: new Date(now.getTime() + 35 * 24 * 60 * 60 * 1000),
        travelers: 2,
        travelerDetails: { firstName: 'Rahul', lastName: 'Sharma', email: 'traveler@tripwise.com', phone: '+91 9123456789' },
        baseFare: 90000,
        taxesAndFees: 10800,
      ...
      },
      {
        ...
        itemId: findDest('Dubai')._id,
        itemName: 'Dubai City Tour',
        itemImage: 'https://images.unsplash.com/photo-1512453979798-5ea266f8880c?w=800&fit=crop&q=80',
        destinationName: 'Dubai, UAE',
        checkIn: new Date(now.getTime() - 20 * 24 * 60 * 60 * 1000),
        checkOut: new Date(now.getTime() - 16 * 24 * 60 * 60 * 1000),
        travelers: 3,
        travelerDetails: { firstName: 'Rahul', lastName: 'Sharma', email: 'traveler@tripwise.com', phone: '+91 9123456789' },
        baseFare: 49000,
        taxesAndFees: 5880,
      ...
      },
      {
        user: travelerUser._id,
        itemType: 'Package',
        itemId: insertedPackages[2]._id,
        itemName: 'Classic Kyoto Experience',
        itemImage: 'https://images.unsplash.com/photo-1493976040374-85c8e12f0c0e?w=800&fit=crop&q=80',
        ...
        discount: 0,
        totalAmount: 98560,
        bookingRef: 'TWKYT2025D',
        status: 'Cancelled',
        paymentStatus: 'Refunded'
      }
    ];
    await Booking.insertMany(bookingsData);
    console.log('[7/8] 4 bookings seeded');
    ...
    await Trip.insertMany([
      {
        ...
        interests: ['Temples', 'Beaches', 'Nature'],
        status: 'planned',
        itinerary: [
          { day: 1, activities: [{ time: '9:00 AM', activity: 'Arrive at Ngurah Rai Airport', notes: 'Transfer to hotel' }] },
          { day: 2, activities: [{ time: '8:00 AM', activity: 'Visit Tegallalang Rice Terraces', notes: 'Bring camera' }] },
          { day: 3, activities: [{ time: '7:00 AM', activity: 'Uluwatu Temple and Kecak Dance', notes: 'Evening show' }] }
        ]
      },
      {
        userId: travelerUser._id,
        ...
        interests: ['Sunsets', 'Wine', 'Fine Dining'],
        status: 'planned',
        itinerary: [
          { day: 1, activities: [{ time: '2:00 PM', activity: 'Arrive in Santorini', notes: 'Sunset welcome' }] },
          { day: 2, activities: [{ time: '10:00 AM', activity: 'Catamaran cruise around the caldera', notes: 'Full day' }] }
        ]
      }
    ]);
    console.log('[8/8] 2 trips seeded');
    ...
    await Notification.insertMany([
      {
        ...
        message: 'Your package booking for Romantic Santorini Getaway (Santorini, Greece) is confirmed. Ref: TWSNT2025A',
      ...
      },
      {
        ...
        type: 'Booking',
        title: 'Booking Confirmed',
        message: 'Your hotel booking for Ubud Hanging Gardens (Bali, Indonesia) is confirmed. Ref: TWBALI2025B',
        actionUrl: '/dashboard/trips',
        read: false
      },
      {
        ...
        message: 'Your booking for Classic Kyoto Experience (Ref: TWKYT2025D) has been cancelled and refunded.',
        read: true
      }
    ]);
    ...
    console.log('===================================');
    console.log('\nDemo Accounts:');
    console.log('  Admin:    admin@tripwise.com    / Admin@123');
    console.log('  Traveler: traveler@tripwise.com / Traveler@123');
    console.log('');
